...
class WMSImageProcessor:
    ...
    def download_image(self,
        ...
        if response.status == 200:    # TODO: change to use try and except
            with open(f"../data/processed/{self.folder_name}_raw/{grid_id}.tif", "wb") as f:
    ...
    def convert2shp(self, code: np.ndarray, meta: dict, grid_id: int | str) -> None:
        ...
        none_vector = []
        with fiona.open(f"../data/processed/{self.folder_name}_gpkg/test.gpkg",
                        "w",
                        "GPKG",
                        gpkg_schema,
                        crs="epsg:3826",
...
if __name__ == "__main__":
    ...
    table_df = pd.read_csv(r"..\data\raw\109年國土利用現況調查成果.csv")
    ...
    grid = gpd.read_file(r"..\data\raw\FET_2023_grid_97.geojson")
    grid_str = list(
